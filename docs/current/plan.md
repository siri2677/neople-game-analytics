...
- 공식 랭킹과 상위 표본의 비교
- 공식 랭킹 API에서 제한된 상위 표본을 자동 수집하며 개인 닉네임·Player ID를 입력받지 않는다.
- 캐릭터별 경기 수·승률
- 평균 킬·도움
- 캐릭터별 아이템 성과
...
- [ ] `.env.example`만으로 사용자가 설정 방법을 이해할 수 있다.
- [ ] 실제 API Key는 GitHub·웹·프론트엔드에 노출되지 않는다.
- [ ] API 없이도 데모 데이터로 웹 화면이 열린다.
- [ ] 실제 수집 데이터로 `transform → web_export`가 재현된다.
- [ ] Overview·던파·사이퍼즈 탭에서 공고와 연결되는 분석 결과를 보여준다.
- [ ] 각 결과에 표본 범위와 한계가 표시된다.
- [ ] Power BI Desktop에서 동일 CSV를 재검증할 수 있다.
- [ ] 외부 유료 Capacity, DB, 인증 서버 없이 공개 URL을 운영할 수 있다.
