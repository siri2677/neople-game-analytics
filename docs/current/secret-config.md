...
따라서 웹사이트를 보여주기 위한 서버 시크릿은 필요하지 않다. API Key는 데이터를 수집할 때만 로컬에서 사용하고, 웹 브라우저에는 전달하지 않는다.
...
| 변수 | 필수 | 용도 | 저장 위치 |
| --- | --- | --- | --- |
| `NEOPLE_API_KEY` | 실제 API 수집 시 필수 | Neople REST API 인증 | 로컬 `.env`만 |
...
```env
NEOPLE_API_KEY=replace_with_your_key
...
즉, 한 번 데이터를 수집·정제·검토해 공개용 `dashboard.json`을 만든 후에는 웹사이트가 API Key 없이도 동작한다.
...
```powershell
rg -n "NEOPLE_API_KEY|POWERBI_CLIENT_SECRET|POSTGRES_PASSWORD|replace_with_.*secret" . -g '!*.pyc' -g '!data/raw/**'
git status --short
```

GitHub Actions로 자동 수집을 추가하는 경우에도 `NEOPLE_API_KEY`는 GitHub Actions Secret으로만 주입하고, 웹 산출물이나 로그에 출력하지 않는다. 현재 계획에는 자동 수집 배포를 필수로 포함하지 않는다.

## 무료 공개 운영 원칙

```text
API Key       = 로컬 수집 단계에서만 사용
